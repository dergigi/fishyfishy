...
val guideFilms: Map<String, GuideFilm> = buildMap {
    listOf("octopus").forEach { id ->
        put(id, GuideFilm("ebeNeQFUMa0", "Extraordinary Octopus Takes To Land | The Hunt | BBC Earth", "BBC Earth", "Octopus relatives", "Watch an octopus explore the shore and hunt. This film shows octopus behaviour, not a Madeira identification lesson."))
    }
    ...
    listOf("toby", "puffer").forEach { id ->
        put(id, GuideFilm("1k0MMxhOVpA", "Pufferfish Builds Sand Sculpture for Mating", "Nature on PBS", "Pufferfish relatives", "A Japanese pufferfish builds a sand nest. This is a different species; Madeira’s puffers do not make these circles."))
    }
    listOf("sardine", "bogue", "sand_smelt").forEach { id ->
        put(id, GuideFilm("0zkvqdw_r4A", "Epic Journey of Sardines | BBC Studios", "BBC Studios", "Schooling fish", "Watch sardines travelling together off South Africa. A comparison of schooling behaviour, not an identification of Madeira’s silver shoals."))
    }
    ...
    listOf("rock_crab", "nimble_crab", "arrow_crab").forEach { id ->
        put(id, GuideFilm("OwQcv7TyX04", "Decorator Crabs Make High Fashion at Low Tide | Deep Look", "Deep Look", "Crab relatives", "Meet decorator crabs that dress in seaweed. These are different crabs; our rock and arrow crabs are not being identified here."))
    }
    ...
    listOf("sponge").forEach { id ->
        put(id, GuideFilm("m8a0oNsDEx8", "Sponges! | JONATHAN BIRD'S BLUE WORLD", "BlueWorldTV", "Sponge relatives", "Learn how sponges live and filter water. A film about sponges in general, not specifically our yellow tube sponge."))
    }
    ...
    listOf("wrasse", "salema", "bream", "zebra", "damselfish", "comber", "grouper", "chromis", "azores_chromis", "saddled", "two_banded", "mullet", "goatfish", "lizardfish", "triplefin", "redlip", "scorpionfish", "black_moray", "moray", "trumpetfish", "triggerfish", "barracuda", "sand_bream", "fireworm", "limpet").forEach { id ->
        put(id, GuideFilm("jVn8ZXaditg", "The Gorgeous Wildlife of the Mediterranean | Free Documentary Nature", "Free Documentary - Nature", "Coastal habitat", "A longer film about Mediterranean sea life. A habitat comparison, filmed outside Madeira; it is not a guide to identifying this species."))
    }
}
